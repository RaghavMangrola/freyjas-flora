...
public interface FarmRunHelperConfig extends Config
{
	...
	default boolean advanceAfterReplant()
	{
		return true;
	}

	@ConfigItem(
		keyName = "waitForCompostBeforeAdvance",
		name = "Wait for compost",
		description = "For compostable farming patches, wait until Time Tracking records compost before advancing",
		section = RUN_PLANNING,
		position = 4
	)
	default boolean waitForCompostBeforeAdvance()
	{
		return true;
	}
